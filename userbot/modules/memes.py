...
async def hacking (hacked):
    ...
    if not hacked.text[0].isalpha() and hacked.text[0] not in ("/", "#", "@", "!"):
        if await hacked.get_reply_message():
            await hacked.edit(
                "`Targetted Account Hacked Successfully 😎 And I Have Your Nudes Now......`\n"
                "`Pay $6969 to` @StudMedico `To Remove This Hack And Prevent To Be Exposed...`\n"
            )
...
async def killing (killed):
    ...
    if not killed.text[0].isalpha() and killed.text[0] not in ("/", "#", "@", "!"):
        if await killed.get_reply_message():
            await killed.edit(
                "`Targetted User Has Been Killed Successfully 😈 Playing 'He Raam' In Background......`\n"
            )
